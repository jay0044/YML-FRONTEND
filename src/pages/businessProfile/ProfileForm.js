...
const KYCPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchKYCStatus = async () => {
      ...
      try {
        const response = await fetch(SummaryApi.getKYC.url(userId), {
        ...
        if (response.ok && result.success) {
          ...
          setSubmissionStatus(
            "Your KYC is already submitted and verification is pending."
          );
        } else {
  ...
  const validateForm = () => {
    if (
      selectedPanCard &&
      (!kycDetails.panNumber || !kycDetails.panName || !kycDetails.panCardFile)
    ) {
    ...
    if (
      ...
      (!kycDetails.aadharNumber ||
        !kycDetails.aadharName ||
        !kycDetails.aadharFile)
    ) {
  ...
  return (
    <div className="relative container mx-auto p-6 bg-white shadow-md rounded-md max-w-5xl">
